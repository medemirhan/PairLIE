...
def run_train(nEpochs, lr, data_train, inp_channels, cLossCoeff, rLossCoeff, pLossCoeff, num_conv_blocks):
    ...
    params.num_conv_blocks = num_conv_blocks
    params.rgb_range = 1
...
if __name__ == '__main__':
    ...
    nEpochs = 80
    lr = 1e-4
    data_train = 'train_ll_skip_bands_outdoor_6_bands'
    data_test = 'test_ll_skip_bands_outdoor_6_bands'
    ...
    rLossCoeff = [75.0, 10.]
    pLossCoeff = [0.0]
    num_conv_blocks = [3, 4, 10]
    luminance_factors = [0.1, 0.5, 0.9, 1.2, 1.8]

    timestamp = f'{datetime.now():{""}%H%M%S}'
    exp_name = 'skip_bands_outdoor_6bands_convBlck'
        ...
        for r in rLossCoeff:
            for p in pLossCoeff:
                for n in num_conv_blocks:
                    model_timestamp, model_dir, model_full_path = run_train(nEpochs, lr, data_train, inp_channels, c, r, p, n)
                    ...
                    for lf in luminance_factors:
                        with mlflow.start_run(nested=False):
                            ...
                            mlflow.log_param("numConvBlocks", n)
